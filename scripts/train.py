...
for class_name in classes:
    ...
    for epoch in range(config["num_epochs"]):
        model.train()
        running_loss = 0.0
        for images, masks, bboxes, class_labels in train_loader:
            images = images.to(device, dtype=torch.float)
            masks = masks.to(device, dtype=torch.float)
            bboxes = bboxes.to(device, dtype=torch.float)
            class_labels = class_labels.to(device, dtype=torch.long)

            optimizer.zero_grad()

            # モデルの予測出力
            bbox_preds, class_preds = model(images, images, images, images)  # 入力に back, seg, multi を仮に images を渡しています

            # デバッグ情報の追加
            print(f"Original class_preds shape: {class_preds.shape}")
            print(f"Original class_labels shape: {class_labels.shape}")

            # クラス予測を2次元に変換する
            class_preds = class_preds.permute(0, 2, 3, 1).reshape(-1, model.num_classes)  # [batch_size * height * width, num_classes]
            class_labels = class_labels.view(-1).repeat(class_preds.shape[0] // class_labels.numel()).long()  # クラスラベルの形状を調整

            # デバッグ情報の追加
            print(f"Reshaped class_preds shape: {class_preds.shape}")
            print(f"Reshaped class_labels shape: {class_labels.shape}")
...
for epoch in range(1):  # 最終エポック1回
    model.train()
    running_loss = 0.0
    for images, masks, bboxes, class_labels in final_train_loader:
        images = images.to(device, dtype=torch.float)
        masks = masks.to(device, dtype=torch.float)
        bboxes = bboxes.to(device, dtype=torch.float)
        class_labels = class_labels.to(device, dtype=torch.long)
        ...
        # クラス予測を2次元に変換する
        class_preds = class_preds.permute(0, 2, 3, 1).reshape(-1, model.num_classes)  # [batch_size * height * width, num_classes]
        class_labels = class_labels.view(-1).repeat(class_preds.shape[0] // class_labels.numel()).long()
